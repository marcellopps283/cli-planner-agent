...
export function buildPlanAnswersFromFreeformRequest(brief: string, dashboard: TuiDashboard): PlanAnswers {
  ...
  return {
    ...
    notes: [
      ...
      "Use the chat as a brainstorming surface: infer safe facts, ask concise questions for important gaps, and keep unresolved architecture choices as options until the user decides.",
...
function buildPlannerAgentWorkflowPrompt({
  ...
  return [
    ...
    "Do not force a plan too early. Move toward preview_plan only after the important product, stack, scope, validation, and constraint gaps are settled enough.",
    ...
    "Treat project_context.stack as repository baseline evidence, not as a lock that limits the user's architectural options.",
    "You may suggest frameworks, packages, libraries, build tools, databases, or test frameworks during brainstorming when they fit the user's goals; present them as options with tradeoffs until the user chooses.",
    ...
    "Prefer dependencies already listed in project_context.declared_dependencies when extending the existing repo, but do not limit a greenfield or redesign conversation to them.",
    "If a new dependency or framework seems useful and the user did not explicitly request it, keep it as a question, option, or pending decision instead of making it a settled plan.",
